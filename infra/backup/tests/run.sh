...
grep -q '.backup' "$repo_root/infra/backup/create-recovery-point.sh" || fail "recovery points must be isolated SQLite .db backups"
...
shasum -a 256 "$1"
FAKESHA
chmod +x "$test_dir/bin/aws" "$test_dir/bin/date" "$test_dir/bin/flock" "$test_dir/bin/sha256sum"
